            ...
            @for ($i = 0; $i < count($search); $i++)
                <div class="card" style="width: 18rem;">
                    ...
                    <div class="card-body">
                        ...
                        <!-- Modal -->
                        <div class="modal fade" id="staticBackdrop{{ $search['results'][$i]['id'] }}"
                                    ...
                                    <div class="modal-body">
                                        ...
                                        @if (isset($listsUser))
                                            @for ($j = 0; $j < count($listsUser); $j++)
                                                <form action="{{ route('home.update', $search['results'][$i]['id']) }}"
                                                    method="post">
                                                    @csrf
                                                    @method("PATCH")
                                                    <input type="hidden" name="list" value="{{ $listsUser[$j]->name }}">
                                                    <button type="submit"
                                                        class="btn btn-secondary">{{ $listsUser[$j]->name }}</button>
                                                </form>
                                            @endfor
                                        @else
